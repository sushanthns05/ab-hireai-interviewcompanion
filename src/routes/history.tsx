    ...
    transcript: [
      {
        ...
        feedback: "Good choice. Token bucket is standard. You could improve this by mentioning how to handle Redis node failures."
      },
      {
    ...
    transcript: [
      {
        ...
        feedback: "The basic idea is there, but you missed handling the edge cases like empty lists or single-node lists."
      },
      {
        q: "What is the time complexity of your approach?",
        a: "It's O(N^2) because we have to visit every node.",
        feedback: "Incorrect. Reversing a linked list linearly is O(N). Review time complexities for basic data structures."
      }
    ]
  }
];
    ...
    >
      ...
      <main className="flex-1 flex flex-col items-center py-12 px-4 sm:px-8 relative bg-black/40">
        <div className="w-full max-w-4xl flex flex-col z-10 relative">
          ...
          >
            <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight mb-3">Session Replay</h1>
            <p className="text-lg text-white/60">Revisit past interviews with full transcripts and feedback to see how you've improved over time.</p>
          </motion.div>

          <div className="flex flex-col gap-6">
            {staticSessions.map((session, i) => {
              const scoreColor = session.score >= 80 ? "text-green-400 bg-green-400/10 border-green-400/20" : 
                                 session.score >= 60 ? "text-amber-400 bg-amber-400/10 border-amber-400/20" : 
                      ...
                      <div className="p-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-3 text-sm text-white/50">
                            <span className="flex items-center gap-1"><Clock className="size-4" /> {session.date}</span>
                            <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/70">{session.mode}</span>
                          </div>
                        </div>
                          ...
                          {session.transcript.map((t, idx) => (
                            <div key={idx} className="flex flex-col gap-4">
                              ...
                              <div className="ml-12 mr-12 bg-white/5 border border-white/10 rounded-xl p-4 flex gap-3 items-start">
                                {t.feedback.includes("Good") || t.feedback.includes("Great") || t.feedback.includes("Excellent") ? 
                                  <CheckCircle2 className="size-5 text-green-400 shrink-0 mt-0.5" /> : 
                                  <AlertCircle className="size-5 text-amber-400 shrink-0 mt-0.5" />
                                }
                                <p className="text-sm text-white/70 leading-relaxed">{t.feedback}</p>
                              </div>
                            </div>
                          ))}
                        </div>
