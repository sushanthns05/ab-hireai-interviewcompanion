...
const features = [
  {
    ...
    description:
      "Questions adjust in real time based on how well you're answering — no fixed script. The AI reads your response quality and recalibrates the next question's difficulty accordingly.",
    icon: Brain,
    color: "teal",
  },
  {
    ...
    description:
      "Get scored on clarity, correctness, and structure right after every answer, not just at the end. See exactly what to improve before you move to the next question.",
  ...
  },
  {
    ...
    description:
      "Tab-switch detection and fullscreen enforcement keep Live Interviews as close to the real thing as possible — no shortcuts, no second tab.",
    icon: ShieldCheck,
    color: "teal",
  },
  {
    ...
    description:
      "Track score trends across sessions and see exactly which topics need more practice, with a breakdown by subject/skill area.",
    icon: BarChart3,
    color: "purple",
    linkTo: "/analytics",
  },
  {
    ...
    description:
      "Revisit past interviews with full transcripts and feedback to see how you've improved over time.",
    icon: History,
    color: "teal",
    linkTo: "/history",
  },
            ...
            {features.map((feature, index) => (
              ...
              >
                <div className="flex-1 flex flex-col items-start text-left gap-6 w-full">
                  <div
                    className={`p-4 rounded-full ${feature.color === "teal" ? "bg-[#2dd4bf]/10 shadow-[0_0_20px_rgba(45,212,191,0.2)]" : "bg-[#c084fc]/10 shadow-[0_0_20px_rgba(192,132,252,0.2)]"}`}
                  >
                    <feature.icon
                      className={`size-8 ${feature.color === "teal" ? "text-[#2dd4bf]" : "text-[#c084fc]"}`}
                    />
                  </div>
                    ...
                    const cardContent = (
                      <>
                        ...
                        <div
                          ...
                          style={{
                            background: `radial-gradient(circle at center, ${feature.color === "teal" ? "rgba(45,212,191,0.15)" : "rgba(192,132,252,0.15)"} 0%, transparent 70%)`,
                          }}
                        />
                        ...
                        {/* Floating Particles */}
                        <div
                          className={`absolute top-1/4 left-1/4 size-1.5 rounded-full ${feature.color === "teal" ? "bg-[#2dd4bf]" : "bg-[#c084fc]"} blur-[1px] opacity-40 animate-pulse`}
                        ></div>
                        <div
                          className={`absolute bottom-1/3 right-1/4 size-2.5 rounded-full ${feature.color === "teal" ? "bg-[#2dd4bf]" : "bg-[#c084fc]"} blur-[1.5px] opacity-30 animate-pulse`}
                          style={{ animationDelay: "1s" }}
                        ></div>
                        <div
                          className={`absolute top-1/2 right-1/3 size-1 rounded-full ${feature.color === "teal" ? "bg-[#2dd4bf]" : "bg-[#c084fc]"} blur-[0.5px] opacity-50 animate-pulse`}
                        ...
                        <div className="relative z-10 flex items-center justify-center">
                          <div
                            className={`absolute inset-0 ${feature.color === "teal" ? "bg-[#2dd4bf]" : "bg-[#c084fc]"} blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 rounded-full`}
                          ></div>
                          <div className="bg-black/60 p-6 sm:p-8 rounded-3xl border border-white/10 backdrop-blur-xl shadow-2xl transition-transform duration-500 group-hover:scale-110 group-hover:-translate-y-2 flex flex-col items-center gap-4">
                            <feature.icon
                              className={`size-16 sm:size-20 ${feature.color === "teal" ? "text-[#2dd4bf]" : "text-[#c084fc]"} drop-shadow-[0_0_15px_rgba(${feature.color === "teal" ? "45,212,191" : "192,132,252"},0.5)]`}
                              strokeWidth={1.5}
                            />
                            {feature.linkTo && (
                              <span
                                className={`text-white font-bold bg-white/5 px-4 py-1.5 rounded-full text-sm backdrop-blur-md border border-white/10 group-hover:bg-white/10 transition-colors shadow-[0_0_15px_rgba(${feature.color === "teal" ? "45,212,191" : "192,132,252"},0.2)]`}
                              >
                                View Demo
                              </span>
                            )}
                        ...
                        <div
                          className={`absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent ${feature.color === "teal" ? "via-[#2dd4bf]" : "via-[#c084fc]"} to-transparent opacity-30 group-hover:opacity-60 transition-opacity duration-500`}
                        ></div>
                      </>
                    );
