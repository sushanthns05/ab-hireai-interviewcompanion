...
const mergeText = (existing: string, addition: string) => {
  ...
  for (let i = 1; i <= Math.min(existingWords.length, additionWords.length); i++) {
    const endExisting = existingWords
      .slice(-i)
      .join(" ")
      .toLowerCase()
      .replace(/[.,?!]/g, "");
    const startAddition = additionWords
      .slice(0, i)
      .join(" ")
      .toLowerCase()
      .replace(/[.,?!]/g, "");
...
export function InterviewRoom({
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => {
      if (document.hidden && !warningMsg)
        handleViolation("Tab switch detected. Please stay focused.");
    };
      ...
      {warningMsg && (
        <div className="fixed inset-0 z-100 bg-black/90 flex flex-col items-center justify-center p-8 text-center backdrop-blur-xl">
          ...
          <p className="text-lg text-white/70 max-w-lg mb-8 leading-relaxed">
            ...
            <br />
            <br />
            Warning {violations}/3: Switching tabs or exiting fullscreen is not allowed during the
            interview. {4 - violations} more violation(s) will end your interview automatically.
          </p>
          <Button
            size="lg"
            variant="destructive"
        ...
        <aside className="space-y-4">
          <div className="panel p-5">
            ...
            <Button
              variant="secondary"
              size="sm"
              className="mt-2 w-full text-destructive hover:text-destructive-foreground hover:bg-destructive"
              onClick={onEndEarly}
            >
              ...
              <dl className="mt-4 space-y-2 font-mono text-xs">
                {debug ? (
                  <>
                    ...
                    <Row
                      label="question"
                      value={`${debug.questionCount}/${debug.targetQuestions}`}
                    />
                    <Row label="difficulty" value={String(debug.difficultyLevel)} />
                    <Row
                      label="curriculum day"
                      value={debug.currentDay ? `Day ${debug.currentDay}` : "—"}
                    />
              ...
              <div className="flex flex-col gap-3 max-w-[85%] animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex items-center gap-3 bg-surface-raised border border-border px-4 py-3 text-sm leading-relaxed text-muted-foreground w-fit rounded-none">
                  ...
                  <span className="animate-pulse">
                    Analyzing candidate response & generating adaptive follow-up...
                  </span>
                </div>
              </div>
            )}
              ...
              <div className="flex items-end gap-3">
                <div className="relative flex-1">
                  <Textarea
                    ...
                    placeholder={
                      isRecording ? "Listening..." : "Type your answer… (⌘/Ctrl + Enter to send)"
                    }
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    size="lg"
                    className="rounded-none shadow-(--shadow-glow)"
                  >
